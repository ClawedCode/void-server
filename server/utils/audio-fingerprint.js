 ...
 */
function extractAudioFingerprint(toneJsCode) {
  if (!toneJsCode) {
    return null;
  }
  ...
  fingerprint.structuralHash = generateStructuralHash(fingerprint);
...
/**
 * Generate a short hash representing the structural elements
 */
function generateStructuralHash(fingerprint) {
  const parts = [
    fingerprint.bpm,
    fingerprint.synthTypes.map(s => s.type).sort().join(','),
    fingerprint.effectTypes.map(e => e.type).sort().join(','),
    fingerprint.loopPatterns.sort().join(',')
  ];
