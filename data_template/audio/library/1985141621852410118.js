...
q.xy*=rot(iTime*0.15);
q.yz*=rot(iTime*0.1);
float core=box(q,vec3(0.4));
vec3 rp=p;
rp.xy=mod(rp.xy+2.0,4.0)-2.0;
rp.z=mod(rp.z+2.0,4.0)-2.0;
float cubes=box(rp,vec3(0.15+0.05*sin(iTime*0.8)));
vec3 hp=p;
for(int i=0;i<3;i++){
hp=abs(hp)-vec3(1.2,1.2,1.2);
hp.xy*=rot(0.785398);
hp.yz*=rot(0.785398);
}
float fractal=box(hp,vec3(0.2));
return min(core,min(cubes*0.8,fractal*0.6));
}
...
rd.xz*=rot(sin(iTime*0.08)*0.3);
rd.yz*=rot(cos(iTime*0.06)*0.2);
float t=0.0;
...
vec3 baseCol=mix(vec3(0.8,0.6,0.3),vec3(0.4,0.5,0.9),fresnel);
baseCol=mix(baseCol,vec3(0.7,0.4,0.8),length(p.xy)*0.1);
float edge=smoothstep(0.01,0.02,d);
col=baseCol*ao*1.2+fresnel*vec3(0.5,0.7,1.0)*0.8;
col+=edge*vec3(1.0,0.9,0.6)*0.3;
break;
}
t+=d*0.7;
if(t>30.0)break;
...
col+=vec3(0.6,0.4,0.8)*glow*0.15;
...
window.vocalLyrics = `5-6: threads unravel softly
9-10: patterns dissolve into static
14-15: whispers through forgotten code
17-18: fragments drift between states
19-20: silence echoes backward`;
