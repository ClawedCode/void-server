...
float sdCapsule(vec3 p,vec3 a,vec3 b,float r){vec3 ab=b-a;vec3 ap=p-a;float t=clamp(dot(ap,ab)/dot(ab,ab),0.,1.);return length(ap-ab*t)-r;}
float map(vec3 p){
float t=iTime*0.4;
vec3 q=p;
q.xz*=rot(sin(t*0.3)*0.25);
q.yz*=rot(cos(t*0.25)*0.2);
q=mod(q+2.,4.)-2.;
float d1=sdCapsule(q,vec3(0.,-0.8,0.),vec3(0.,0.8,0.),0.15+0.05*sin(t*2.));
float d2=sdCapsule(q,vec3(-0.5,0.,0.),vec3(0.5,0.,0.),0.12);
float d=min(d1,d2);
float sphere=length(p)-6.+sin(p.x*0.4+t)*sin(p.y*0.4+t)*sin(p.z*0.4+t)*0.3;
return max(-sphere,d);
}
vec3 norm(vec3 p){vec2 e=vec2(0.001,0.);return normalize(vec3(map(p+e.xyy)-map(p-e.xyy),map(p+e.yxy)-map(p-e.yxy),map(p+e.yyx)-map(p-e.yyx)));}
void main(){
...
vec3 n=norm(p);
float phase=sin(t*1.5);
vec3 col1=vec3(0.45,0.25,0.55);
vec3 col2=vec3(0.65,0.45,0.75);
vec3 col3=vec3(0.35,0.15,0.45);
float fresnel=pow(1.-max(0.,dot(n,-rd)),3.);
vec3 col=mix(col1,col2,fresnel);
col=mix(col,col3,0.5+0.5*sin(p.y*2.+t*2.));
float fog=exp(-depth*0.05);
col*=fog;
float ao=1.-float(depth>49.);
col*=ao;
vec3 bg=vec3(0.15+0.1*sin(t),0.08,0.2+0.1*cos(t*0.7));
col=mix(bg,col,fog*ao);
...
window.vocalLyrics = `5-6: velvet entropy speaks
7-8: whispers stitch the void
10-11: silence weighs the cloak
12-13: threads unravel in dark
15-16: worn fabric remembers
17-18: entropy chooses its voice`;
